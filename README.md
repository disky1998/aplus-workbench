...
抓竞品 / 自己 ASIN 的 A+ 图文内容，覆盖**普通 A+ 与高级 A+（Premium A+）**，同时出**桌面端**和**移动端**两份结构。

品牌故事（Brand Story）默认不抓，需要时加 `--include-brand-story`。底部关联推荐位（`aplusRelatedContent` / `aplusDetailPageBtfFeaturedContent`）不算 A+ 内容，已排除。

---

## 下载即用（推荐）

到 [Releases](https://github.com/disky1998/aplus-workbench/releases/latest) 下载 **`APlusWorkbench.exe`**，双击即可：
...
> **首次抓取建议勾选「有头模式」跑一次**：在弹出窗口里过掉验证码，会话会被记住，之后就不容易再被拦。
> 页面会检测登录态并在标题旁显示「登录态就绪」。
